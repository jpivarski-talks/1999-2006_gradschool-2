...
(defconst py-version "$Revision: 4.54-jimp $"
...
(defcustom py-python-command "/nfs/cleo3/Offline/rel/20050721_P2/other_sources/python/bin/python2.4"
...
(defcustom py-jump-on-exception nil
  ...
  (let ((kw1 (mapconcat 'identity
    ...
    (list
     ...
     '("\\bdef[ \t]+\\([a-zA-Z_]+[a-zA-Z0-9_]*\\)"
       1 font-lock-function-name-face)
     ;; pseudo-keywords
; JimP 26 Mar 2005: I stuck in "other" because I use that in contrast to "self"
     '("\\b\\(self\\|other\\|None\\|True\\|False\\|Ellipsis\\)\\b"
...
(if py-mode-map
  ...
  ;; subprocess commands
; JimP 14 Feb 2005: I just had to swap these because of the way I'm accustomed to invoking an -execute-string
  (define-key py-mode-map "\C-c\C-s"  'py-execute-buffer)
  (define-key py-mode-map "\C-c\C-c"  'py-execute-string)
; JimP 26 Mar 2005: and add these two:
  (define-key py-mode-map "\C-c\C-a"  'py-jimp-abort)
  (define-key py-mode-map "\C-c\C-q"  'py-jimp-kill)
;
...
; JimP 12 Feb 2005: they forgot this one:
  ...
  (define-key py-mode-map "\C-j" 'py-jimp-execute-statement)
  )
...
; JimP 10 May 2005: I can't stand this!!!
...
; JimP 12 Feb 2005
(defvar py-last-input nil
  "To keep track of the point in the output buffer that the window should scroll to")

(defvar py-how-many-times 0
  "To keep track of whether or not to scroll the window")

(defvar py-jimp-input-face 'py-jimp-input-face
  "Face name to use for input written on the python log buffer (comint).")

(defface py-jimp-input-face
...

;;;###autoload
(defun python-mode (&optional jimp)
...
; JimP 12 Feb 2005:
  (make-local-variable 'beginning-of-stdin)
  ;;
  ...
  ; JimP 10 May 2005: Just because this annoys me
  ...
  (when (null py-which-shell)
    (py-toggle-shells (py-choose-shell) jimp)))
...
(defun py-execute-file (proc filename &optional jimp)
...
;	(msg (if jimp (format "\n") (format "## working on region in file %s...\n" filename)))
	(msg (py-jimp-printout-command filename))
...
; JimP 12 Feb 2005:
	  (setq py-last-input (point))
	  (set-window-start (get-buffer-window (current-buffer) t) py-last-input)
	  (setq py-how-many-times (1+ py-how-many-times))
; end JimP
	  (funcall (process-filter proc) proc msg)
; JimP 11 Mar 2005:
;; 	  (let ((over (make-overlay (- py-last-input 4) (point))))
;; 	    (when comint-highlight-prompt
;; 	      (overlay-put over 'face 'py-jimp-input-face)
;; 	      ))
; end JimP
	  )
      (set-buffer curbuf))
...
(defun py-jimp-comint-move-window-function (string)
  "This just moves the window position to py-last-input, if it exists."
  (if (and py-last-input (> py-how-many-times 0))
      (progn
	(set-window-start (get-buffer-window (current-buffer) t) py-last-input)
	(setq py-how-many-times (1- py-how-many-times))
	))
...
(defun py-jimp-printout-command (filename)
  "Print the input into the log buffer, so that the user can follow changes."
  (let ((curbuf (current-buffer))
	(tmpbuf (generate-new-buffer "*Python-jimp-temp*"))
	(copyover))
    (set-buffer tmpbuf)
    (insert-file-contents filename)
    (goto-char (point-min))
    (end-of-line)
    (if (not (= (point) (point-max))) (forward-char) (insert "\n"))
    (while (not (= (point) (point-max)))
      (insert "... ")
      (end-of-line)
      (if (not (= (point) (point-max))) (forward-char) (insert "\n"))
      )
    (setq copyover (buffer-substring (point-min) (point-max)))
    (set-buffer curbuf)
    (kill-buffer tmpbuf)
    copyover
    ))
...
; JimP 20 Sep 2005: This was NEVER correct for ipython.
;                   I have NO IDEA how ipython people use this.
...
;    (pop-to-buffer (current-buffer))
    (py-postprocess-output-buffer (current-buffer) py-last-input)
;
...
(defun py-postprocess-output-buffer (buf &optional jimp)
...
If an exception occurred return t, otherwise return nil.  BUF must exist."
  (let (line file bol err-p)
    (save-excursion
      (set-buffer buf)
; JimP 14 Feb 2005:
;      (if (not jimp) (beginning-of-buffer) (goto-char jimp) (message (format "on %s" jimp)))
; JimP 11 Mar 2005: I don't understand it.  That message must have
; been debugging code, because I don't know why else it would be here.
      (if (not jimp) (beginning-of-buffer) (goto-char jimp))
...
(defun py-toggle-shells (arg &optional jimp)
...
`jpython', equivalent to positive arg and negative arg respectively."
  ...
  (let (msg)
    ...
    (if (not jimp) (message "Using the %s shell" msg))
...
(defun py-jimp-abort ()
  ...
  (interactive)
  (interrupt-process (get-process py-which-bufname))
  )

(defun py-jimp-kill ()
  "Kill (quit) the python process."
  (interactive)
  (kill-process (get-process py-which-bufname))
  )

(defun py-jimp-execute-statement ()
  "Execute a single statement, even if it is a block.  I introduce an
&optional jimp hack into (py-execute-region), (py-execute-file),
(python-mode), and (py-toggle-shells) to keep it from writing a lot of
messages.  This does nothing in comments."
  (interactive)

  (let ((initial (point)) (statements-left 0) start end)
    (while (py-continuation-line-p) (previous-line 1))
    (beginning-of-line)
    (if (not (= (point-min) (point)))
	(progn
	  (backward-char)
	  (setq statements-left (py-next-statement 1))
	  )
;      (progn
;	(setq statements-left (py-next-statement 1))
...
;	)
	...
	(goto-char initial)
      ...
      (if (looking-at "\n") (goto-char end))
      (py-execute-region start end nil t)
      )
    )
  )
...
filter."
  ...
  (let ((args py-which-args) original-buffer)
    (when (and argprompt
	       (interactive-p)
...
; JimP 12 Feb 2005
    (add-hook 'comint-output-filter-functions 'py-jimp-comint-move-window-function)
...

(defun py-execute-region (start end &optional async jimp)
...
is inserted at the end.  See also the command `py-clear-queue'."
  (interactive "r\nP")
; JimP 12 Feb 2005:
  (setq beginning-of-stdin (count-lines 1 start))
...
;	(pop-to-buffer buf)
	...
	(write-region (point-min) (point-max) file nil 'nomsg))
	  ...
	  (py-execute-file proc file jimp)
...
(defun py-jump-to-exception (file line)
  ...
  (if (not (file-exists-p file))
      (message "Exception on line %d of previously-executed code" line)
    (let ((buffer (cond ((string-equal file "<stdin>")
	  ...
	  (python-mode))
; JimP 12 Feb 2005:
      (if (and (boundp 'beginning-of-stdin)
	       beginning-of-stdin) (setq line (+ beginning-of-stdin line)))
      (goto-line line)
