...
PyObject* docontours(PyObject *self, PyObject *args)
{
   ...
   PyObject *p_cont = PyList_New(cont.size());
...
PyObject* doscan(PyObject *self, PyObject *args)
{
   ...
   if (!PyArg_ParseTuple(args, "OiOOOOdiiOidd", &p_fcn, &npar, &p_name, &p_start, &p_step, &p_limits, &eps, &maxcalls, &strategy, &p_covariance, &parnum, &low, &high)) {
      PyErr_SetString(PyExc_TypeError, "calling format must be: FCN(f), npar(i), names(l of s), start(l), step(l), limits(l), eps(d or 0), maxcalls(i or 0), strategy(i), covariance(l or None), parnum(i), low(d or 0), high(d or 0)");
      return NULL;
   }
   ...
   try {
      ...
      PyObject *p_s = PyList_New(s.size());
      if (p_s == NULL) return NULL;
      for (int i = 0;  i < s.size();  i++) {
	 PyObject *obj0 = PyList_New(2);
	 if (obj0 == NULL) {Py_DECREF(p_s); return NULL;}
	 PyObject *obj1 = PyFloat_FromDouble(s[i].first);
	 if (obj1 == NULL) {Py_DECREF(p_s); return NULL;}
	 PyObject *obj2 = PyFloat_FromDouble(s[i].second);
	 if (obj2 == NULL) {Py_DECREF(p_s); return NULL;}

	 if (PyList_SetItem(obj0, 0, obj1) != 0) {Py_DECREF(p_s); return NULL;}
	 if (PyList_SetItem(obj0, 1, obj2) != 0) {Py_DECREF(p_s); return NULL;}
	 if (PyList_SetItem(p_s, i, obj0) != 0) {Py_DECREF(p_s); return NULL;}
      }	 

      PyObject *output = Py_BuildValue("O", p_s);
      Py_DECREF(p_s);
      delete mnapp;
      return output;
   }
