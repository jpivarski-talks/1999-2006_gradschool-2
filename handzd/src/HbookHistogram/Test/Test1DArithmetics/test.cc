...
// $Id: test.cc,v 1.5 2003/08/18 20:21:32 cdj Exp $
//
// Revision history
//
// $Log: test.cc,v $
// Revision 1.5  2003/08/18 20:21:32  cdj
// work around g++ bug which thinks 2/7 != 2/7
//
// Revision 1.4  2002/08/16 15:00:56  cleo3
...
static const char* const kIdString  = "$Id: test.cc,v 1.5 2003/08/18 20:21:32 cdj Exp $";
	      ...
	      Op op)
{
   ...
   for( unsigned int bin = 1; bin <= kNChannels; ++bin ) {
     //a bug in g++ requires us to use a temporary or else
     // the divide_check_0 test fails because 2/7 doesn't equal 2/7!
     float result = op( iFirst->channelValue(bin), 
			iSecond->channelValue(bin) );
     if( iResult->channelValue( bin ) != result ) {
	cout <<"bin "<<bin<<" result "<<iResult->channelValue(bin)<<"\n"
	     <<" first "<<iFirst->channelValue(bin)
	     <<" second "<<iSecond->channelValue(bin)
	     <<"\n op "<< result 
	     <<" diff "<< iResult->channelValue(bin) -
	     op(iFirst->channelValue(bin), iSecond->channelValue(bin))<<endl;
	 return false;
      }
   }
