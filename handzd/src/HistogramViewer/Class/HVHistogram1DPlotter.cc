...
// $Id: HVHistogram1DPlotter.cc,v 1.3 2002/07/19 17:48:09 cdj Exp $
//
// Revision history
//
// $Log: HVHistogram1DPlotter.cc,v $
// Revision 1.3  2002/07/19 17:48:09  cdj
// Now properly draws the last bin.
//
...
static const char* const kIdString  = "$Id: HVHistogram1DPlotter.cc,v 1.3 2002/07/19 17:48:09 cdj Exp $";
...
HVHistogram1DPlotter::retrieveNewValues()
{
   ...
   double* x = new double[numberOfChannels+1];
   double* y = new double[numberOfChannels+1];
   ...
   //stop it from suppressing the 0 and draw the last bin
   // all the way to its end
   y[numberOfChannels] = 0;
   x[numberOfChannels] = m_histogram->highEdge();

   setCurveData( curveId(), x,y,numberOfChannels+1);
   replot();
