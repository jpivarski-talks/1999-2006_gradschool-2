...
// $Id: HVHistogramsListView.cc,v 1.4 2002/08/09 18:16:04 cdj Exp $
//
// Revision history
//
// $Log: HVHistogramsListView.cc,v $
// Revision 1.4  2002/08/09 18:16:04  cdj
...
static const char* const kIdString  = "$Id: HVHistogramsListView.cc,v 1.4 2002/08/09 18:16:04 cdj Exp $";
...
{
   if( HVItemBase::kHist1D == static_cast<HVItemBase*>(iItem)->type() ) {
      emit doubleClicked(static_cast<HVItemHist1D*>(iItem)->histogram() );
   }
   if( HVItemBase::kHist2D == static_cast<HVItemBase*>(iItem)->type() ) {
      emit doubleClicked(static_cast<HVItemHist2D*>(iItem)->histogram() );
   }
   if( HVItemBase::kProfile == static_cast<HVItemBase*>(iItem)->type() ) {
      emit doubleClicked(static_cast<HVItemHistProf*>(iItem)->histogram() );
   }
}
...
{
   if( HVItemBase::kHist1D == static_cast<HVItemBase*>(iItem)->type() ) {
      emit returnPressed(static_cast<HVItemHist1D*>(iItem)->histogram() );
   }
   if( HVItemBase::kHist2D == static_cast<HVItemBase*>(iItem)->type() ) {
      emit returnPressed(static_cast<HVItemHist2D*>(iItem)->histogram() );
   }
   if( HVItemBase::kProfile == static_cast<HVItemBase*>(iItem)->type() ) {
      emit returnPressed(static_cast<HVItemHistProf*>(iItem)->histogram() );
   }
}
//
